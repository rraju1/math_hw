...
\begin{enumerate}
	...
	\item We need to prove that if $y - x > \sqrt{2}, \exists$ an integer $m$ s.t. $x < m\sqrt{2} < y.$ Let $m$ be the smallest positive integer such that $m\sqrt{2} > nx.$ $x < \frac{m\sqrt{2}}{n} < y$; since $nx<m\sqrt{2}$ by definition of $m$, $(m - 1)\sqrt{2} < nx.$ On the other hand, $nx < ny - \sqrt{2}$ so $(m-1)\sqrt{2} < nx < ny - \sqrt{2}.$ This reduces to $\sqrt{2}m < nx < ny \implies \sqrt{2}m < ny.$ This finishes the proof. 
\end{enumerate}
\end{solution}
