...
\begin{enumerate}
	\itemsep0em
	\item Let $A_1$ be the set $(a, b + \epsilon_1)$ for some $\epsilon_1 > 0.$ This translates to $\{x\in \overline{\R} : a < x < b + \epsilon_1\}.$ Since this is a general intersection, another set in the collection will some subset, $A_2\subset A_1, A_2=\{x\in \overline{\R} : a < x < b + \epsilon_2 \}$ where $\epsilon_1 > \epsilon_2$. There are many more subsets of $A_2$ and will continue for $\epsilon_{n+1} < \epsilon_{n}, n \in \mathbb{N}.$ The key insight to note is that $b$ is a common element between all subsets. Clearly, the upper bounds of these subsets are tending to $b$ so that will be the upper bound of general intersection. So, $A_1\cap A_2 \cap \dots = \{x\in \overline{\R} : a < x \leq b \}=(a,b].$
	\item 
	\item 
\end{enumerate}
\end{solution}
