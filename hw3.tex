...
\end{problem}
\begin{solution}
For $x \in U$, since $U\subset X$ and $(X,d)$ is a metric space, $B_X(x, r)$ is open in $X.$ Since $U \subset X, \Int_X(B_X(x, r))\subset \Int_U(B_X(x, r)).$ So every $B_X(x, r)$ is an interior point of $U$. $U = \cup_{x\in U} B_X(x, r) = \Int_X U$ by Prop 2.17(1) to prove $U$ is open with respect to $X.$
\end{solution}
